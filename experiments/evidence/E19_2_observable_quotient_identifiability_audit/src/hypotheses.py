"""Graph-Hodge hypothesis basis construction for OQCI.

Reuses and extends E19.1 basis.py with:
- graph modes (smooth sheet/net currents)
- via modes (vertical spots + sheet compensation)
- gap modes (registration derivatives, standoff laplacians, drift)
- return modes (multi-position loops, edge, distributed)

Also includes gap model reference patterns (from E19.1 gap_models.py).
"""
...
def _forward_block(bundle: OperatorBundle, block: BasisBlock) -> np.ndarray:
    ...
    # Observation-space block: tile for each height if multi-height
    n_heights = max(1, len(bundle.heights) if bundle.heights else 1)
    obs_per_height = block.matrix.shape[0]
    if n_heights > 1 and bundle.A.shape[0] == n_heights * obs_per_height:
        return np.tile(block.matrix, (n_heights, 1))
    return block.matrix
...
def _return_loop_pattern(bundle: OperatorBundle, layer: int, amplitude: float, cx: float, cy: float) -> np.ndarray:
    ...
    top_val = max(0, i_cy - margin)
    bottom = min(n, i_cy + margin)
    if top_val < n and left < right:
        jx[top_val, left:right] += amplitude
    if bottom < n and left < right:
        jx[bottom, left:right] -= amplitude
    if 0 < right and top_val < bottom:
        jy[top_val:bottom, left] -= amplitude
    if right < n and top_val < bottom:
        jy[top_val:bottom, right] += amplitude
    x = empty_current(bundle)
...
def _all_admissible_basis(bases: dict[str, HypothesisBasis], bundle: OperatorBundle) -> np.ndarray:
    """Stack all hypothesis bases into one big current basis."""
    parts = []
    for h in HYPOTHESES:
    ...
    return np.zeros((bundle.A.shape[1], 0))


# ── gap model reference patterns (from E19.1 gap_models.py) ──

def registration_pattern(n: int, center: tuple[float, float], sigma_cells: float, amplitude: float) -> np.ndarray:
    yy, xx = np.mgrid[0:n, 0:n]
    cx, cy = center
    g = np.exp(-((xx - cx) ** 2 + (yy - cy) ** 2) / (2.0 * sigma_cells ** 2))
...
def standoff_laplacian_pattern(clean_field: np.ndarray, amplitude: float) -> np.ndarray:
