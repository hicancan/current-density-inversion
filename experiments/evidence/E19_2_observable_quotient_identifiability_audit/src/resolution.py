...
Computes the resolution operator R = A+ A (or regularized variants) and
reports how much of the current state is recoverable from magnetic observations.
"""
...
def resolution_operator(bundle: OperatorBundle, alpha: float = 0.0) -> np.ndarray:
    """Compute the resolution operator R = (A^T A + alpha I)^(-1) A^T A.

    If R is close to identity, the current state is well-resolved.
    If R is far from identity, only projected/smoothed components are recoverable.
    """
    A = bundle.A
    AtA = A.T @ A
    if alpha > 0:
...
def resolution_diagnostics(bundle: OperatorBundle, alpha: float = 1e-6) -> dict:
    """Compute resolution diagnostics.

    Returns:
    - resolution_trace: trace(R) / dim (close to 1 = well-resolved)
    - resolution_rank: number of eigenvalues of R above 0.5
    - psf_diagonals: representative PSF widths (diagonal vs off-diagonal energy)
    """
    ...
    # Representative PSF: pick a few columns and measure their spread
    n_sample = min(10, dim)
    indices = np.linspace(0, dim - 1, n_sample, dtype=int)
    psf_widths = []
    for i in indices:
        col = R[:, i]
        # Measure diagonal dominance: value at center vs total energy
        diag_val = abs(col[i])
...
def effective_observable_rank(bundle: OperatorBundle) -> dict:
    """Compute effective rank via SVD of the forward operator.
