"""Forward operators for E19.2 OQCI with multi-height stacking.

Reuses E19.1's deterministic four-layer Biot-Savart operator and adds
multi_height_operator_stack() for experiments at multiple sensor heights.
"""
...
class OperatorBundle:
    ...
    A_per_height: dict[float, np.ndarray] | None = None
...
def multi_height_operator_stack(cfg: dict) -> OperatorBundle:
    """Build a multi-height operator stack.

    For each height in sensor_heights_um, builds A_h and stacks them:
    A_E = [A_h1; A_h2; ...; A_hN]

    Returns a single OperatorBundle with the stacked A and per-height info.
    """
    n = int(cfg["grid_size"])
    ...
    per_height: dict[float, np.ndarray] = {}
    ...
    for h in heights:
        xy = _grid_positions(n, pitch)
        cell_count = n * n
        obs_positions_parts.append(
            np.column_stack([xy[:, 0], xy[:, 1], np.full(cell_count, h * 1e-6)])
        )
...
def build_default_operator(cfg: dict) -> OperatorBundle:
    """Build operator bundle from config (convenience wrapper)."""
